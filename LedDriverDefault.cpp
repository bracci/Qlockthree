  ...
  **************/
  ...
  for (byte k = 0; k < _linesToWrite; k++) {
    ...
    if (matrix[k] != 0) {
      ...
      if (_displayOn == true) {
        if ((TRANSITION_MODE_FADE == _settings->getTransitionMode()) && !_transitionCompleted) { // �ber OE einschalten und nach PWM-Anteil wieder ausschalten, wenn das Display aktiv ist
          // Alter Zeileninhalt
          // Zeile schreiben...
